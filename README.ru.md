...
## Тюнинг ресурсов (CPU/RAM)

Можно настроить потребление памяти/CPU при индексации без потери качества поиска:

- `FILE_SCAN_BATCH_SIZE` (по умолчанию: `50`) — сколько файлов сканируется/чанкуется параллельно.
- `INDEX_BATCH_SIZE` (по умолчанию: `50`) — максимум файлов в одной embed/upsert-батч операции.
- `INDEX_MAX_CHUNKS_IN_MEMORY` (по умолчанию: `1200`) — лимит буфера чанков в памяти до принудительного flush.
- `EMBEDDING_BATCH_SIZE` (по умолчанию: `32`) — сколько текстов отправляется в embedding-провайдер за один запрос.

Для маломощных машин уменьшите значения (например, `20/20/400/16`).
